...
const setRevokeAtNowByToken = `-- name: SetRevokeAtNowByToken :exec
UPDATE refresh_tokens 
SET updated_at = NOW(), revoked_at = NOW()
WHERE token = $1
`

func (q *Queries) SetRevokeAtNowByToken(ctx context.Context, token string) error {
	_, err := q.db.ExecContext(ctx, setRevokeAtNowByToken, token)
	return err
}
